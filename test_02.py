#Usage: python test_02.py <id> <model_name> <sample_input.txt-name of input file> 

# python test_02.py "12qw" "model1" sample_input.txt
# python test_02.py "34er" "model_2" sample_input.txt
...
model_path = "best_model_run_msg1ou0e.pth"
model = ConvNet1D()
...
if __name__ == "__main__":
    ...
    try:
        ...
        y_pred = np.array([reverse_scaling_all(data) for data in y_pred])
        ...
        # Create JSON output
        predictions_json = [{params[j]: round(pred[j],4) for j in range(len(params))} for pred in y_pred]
        # for pred_dict in predictions_json:
        #     print(pred_dict)

        # Save to JSON file
        with open(f"{id}.json", 'w') as json_file:
            json.dump(predictions_json[0], json_file, indent=2)
